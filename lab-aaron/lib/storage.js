...
storage.update = function(itemId, item) {
  ...
  return new Promise((resolve, reject) => {
    if(!itemId) return reject(createError(400, 'cannot update item: schema required'));
    if(!item) return reject(createError(400, 'cannot update item: updated item required'));
    if(item._id !== itemId) return reject(createError(400, 'cannott update item ids do not match'));
